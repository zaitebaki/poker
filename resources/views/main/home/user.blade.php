<main-home-component :name-project="{{ json_encode(__('main_page_content.nameProject'), JSON_UNESCAPED_UNICODE) }}"
  ...
  :friends="{{ $friends }}" :user="{{ Auth::user() }}"
  ...
  status="{{ session('status') }}" session-status-user-login="{{ session('sessionStatusUserLogin') }}">
</main-home-component>
<hr>
